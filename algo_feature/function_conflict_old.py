...
def get_students_with_schedule_conflicts(Data):
    ...
    query = """
        SELECT 
            lgs1.ID_STUDENT,
            c1.ID_AVAILABILITY,
            c1.ID_COURSE AS COURSE_1,
            c2.ID_COURSE AS COURSE_2,
            c1.PROMO AS SCHOOL_YEAR_1,
            c2.PROMO AS SCHOOL_YEAR_2
        FROM 
            List_Groups_Students lgs1
        JOIN 
            (SELECT ID_COURSE, ID_AVAILABILITY, PROMO FROM Courses ORDER BY LENGTH(ID_COURSE), ID_COURSE) AS c1 
            ON lgs1.ID_COURSE = c1.ID_COURSE
        JOIN 
        ...
        WHERE 
            ...
            AND c1.ID_COURSE < c2.ID_COURSE
        ORDER BY 
            c1.ID_AVAILABILITY, LENGTH(c1.ID_COURSE), c1.ID_COURSE;
    """
    ...
    for conflict in conflicts:
        student_id, availability, course_1, course_2, school_year_1, school_year_2 = conflict
        # Process conflicts to ensure the English course (if present) is the first in the tuple
        if 'ANG' in course_1:
            processed_conflicts.append((student_id, availability, course_1, course_2, school_year_1, school_year_2))
        elif 'ANG' in course_2:
            processed_conflicts.append((student_id, availability, course_2, course_1, school_year_2, school_year_1))
        else:
            processed_conflicts.append((student_id, availability, course_1, course_2, school_year_1, school_year_2))
 
    # Function to sort conflicts by availability and course number
    def sort_key(x):
        availability, course_1, school_year_1 = x[1], x[2], x[4]
    ...
    sorted_conflicts = sorted(processed_conflicts, key=sort_key)  # Sort conflicts using the sort_key function
    print(sorted_conflicts)
    return sorted_conflicts  # Return the sorted conflicts
...
def get_students_in_conflict(course_id, Data):
    conn = sqlite3.connect(Data)  # Connect to the SQLite database
    cursor = conn.cursor()  # Create a cursor object to execute SQL commands
    
    # SQL query to get the student IDs in conflict for a specific course
    query_students = """
        SELECT 
            DISTINCT lgs1.ID_STUDENT
        FROM 
            List_Groups_Students lgs1
        JOIN 
            Courses c1 ON lgs1.ID_COURSE = c1.ID_COURSE
    ...
    print(student_ids)  # Fetch all results
    
    # Close the connection for the second query
    conn.close()
...
def get_list_group(Data, language, promo_list):
    ...
    cursor.execute("SELECT ID_COURSE FROM Courses WHERE LANGUAGE LIKE ? AND ID_COURSE like ?;", (language, result_str,))
...
def get_promo_list(Data, id_availability, id_group):
    ...
    conn = sqlite3.connect(Data)  # Connect to the SQLite database
    cursor = conn.cursor()  # Create a cursor object to execute SQL commands
    cursor.execute("SELECT PROMO FROM Courses WHERE ID_COURSE LIKE ? AND ID_AVAILABILITY LIKE ?;", (id_group, id_availability,))
    result = cursor.fetchall()  # Fetch all results
    conn.close()  # Close the database connection
